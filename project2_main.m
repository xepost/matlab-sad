...
for n_index = 1:length(n)
   ...
   state_vector_errors(n_index,:,1) = V(:,1) - 0;
   state_vector_errors(n_index,:,2) = V(:,2) - n(n_index);
   state_vector_errors(n_index,:,3) = V(:,3) - 0;
   state_vector_errors(n_index,:,4) = V(:,4) - 0;
   state_vector_errors(n_index,:,5) = V(:,5) - 0;
   state_vector_errors(n_index,:,6) = V(:,6) - 0;
   state_vector_errors(n_index,:,7) = V(:,7) - 1;
...
project2_plotting(nu,state_vector,n, psi)
